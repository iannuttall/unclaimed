import { checkDomain } from "./resolvers";
...
export const TRACKED_TLDS = [
  // tech / startup
  "com", "co", "io", "ai", "dev", "app", "sh", "so", "xyz", "me",
  "tech", "cloud", "online", "site", "space", "world", "digital",
  "systems", "network", "tools", "codes", "software", "build", "run", "now", "works",
  // web / content
  "page", "link", "wiki", "blog", "email", "studio", "design", "art",
  "media", "news", "photos", "video", "audio", "film", "show", "music",
  // commerce
  "store", "shop", "market", "deals", "sale", "gifts", "money", "fund",
  "finance", "capital", "ventures", "exchange", "trade", "games",
  // community / brand
  "club", "team", "group", "agency", "life", "live", "love", "fun",
  "guru", "expert", "pro", "zone",
  // lifestyle / misc + ccTLDs
  "health", "fit", "food", "travel", "ink", "plus", "md", "to",
];
...
export class NoDbError extends Error {
  constructor() {
    super(
      "tracking requires a D1 binding named DB. See README → Tracking setup.",
    );
  }
}
...
export async function listColdOutreach(
  env: TrackerEnv,
  limit = 200,
): Promise<DomainRow[]> {
  const { results } = await db(env)
...
export async function listDropping(
  env: TrackerEnv,
  limit = 200,
): Promise<DomainRow[]> {
  const { results } = await db(env)
...
export async function stats(env: TrackerEnv): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    byStatus: Object.fromEntries(
      (byStatus.results ?? []).map((r) => [r.status, r.n]),
    ),
