...
export interface ResolverConfig {
  /** Per-TLD RDAP URL. Use {domain} where the full domain should be inserted. */
  rdap?: Record<string, string>;
  /** Per-TLD WHOIS server overrides. */
  whois?: Record<string, string>;
  /** Extra case-insensitive WHOIS phrases that mean a name is available. */
  availablePatterns?: Record<string, string[]>;
  /** Minimum delay between requests to a WHOIS server. */
  whoisPaceMs?: Record<string, number>;
}

export function configureResolvers(config: ResolverConfig): void {
  for (const [tld, template] of Object.entries(config.rdap ?? {})) {
    RDAP_OVERRIDES[tld.toLowerCase()] = (domain) =>
      template.replaceAll("{domain}", encodeURIComponent(domain));
  }
  for (const [tld, server] of Object.entries(config.whois ?? {})) {
    WHOIS_OVERRIDES[tld.toLowerCase()] = server;
  }
  for (const [tld, patterns] of Object.entries(config.availablePatterns ?? {})) {
    AVAILABLE_PATTERNS[tld.toLowerCase()] = patterns.map((pattern) => new RegExp(pattern, "i"));
  }
  Object.assign(WHOIS_PACE_MS, config.whoisPaceMs ?? {});
}
...
async function discoverWhoisServer(tld: string): Promise<string> {
  ...
  discoveredWhois.set(tld, server);
  return server;
}
...
export async function checkDomain(
  ...
  const configuredSuffixes = new Set([
    ...Object.keys(RDAP_OVERRIDES),
    ...Object.keys(WHOIS_OVERRIDES),
    ...Object.keys(AVAILABLE_PATTERNS).filter((key) => key !== "default"),
  ]);
  const fallbackTld = labels.at(-1);
  if (!fallbackTld) throw new Error(`Invalid domain: ${domain}`);
  const tld =
    labels
      .slice(1)
      .map((_, index) => labels.slice(index + 1).join("."))
      .find((suffix) => configuredSuffixes.has(suffix)) ?? fallbackTld;
  const checkedAt = new Date().toISOString();

  const finalize = (status: Status, src: Source, expiry: string | null): CheckResult => ({
    domain,
    tld,
    status,
    source: src,
